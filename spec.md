...
* **App NFC read (`client/mobile/src/services/nfc.ts`)**: two runtime-detected backends, so the web bundle keeps no hard dependency. (1) **Native** `@capawesome-team/capacitor-nfc` (`Capacitor.Plugins.Nfc`) — the only path that works inside the APK, since Android's System WebView disables Web NFC (`NDEFReader` → "NFC permission request denied"). Enable it with `npm i @capawesome-team/capacitor-nfc && npx cap sync android` in `mobile/` (no code import needed — detection is runtime-only). (2) **Web NFC** (`NDEFReader`) fallback where the browser grants it. The tullave chip is an encrypted MIFARE DESFire whose balance sits in a key-protected file the app cannot read, so NFC yields only the tag **serial (UID)** and any NDEF records — surfaced as `source:"card"` and NEVER as a verified balance. If NDEF exposes a card number the app auto-consults the server ledger; the balance stays server-sourced.
