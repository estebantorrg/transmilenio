...
### 5.9 Voice — "¿qué tan cerca está el F19?"

A rider who has just unlocked their phone should be able to say one thing and hear one sentence back. Bare `"Hey Google, how close is the F19?"` does **not** reach a third-party app: Assistant's App Actions voice shortcuts do not survive the Gemini transition, and their successor — AppFunctions (`androidx.appfunctions`) — needs Android 16 (API 36, our `compileSdk` is 34, pinned by the phonegap-nfc patch) *and* an allowlist that is still private preview. So the shipped entry point is the stock `"Hey Google, abre <app>"` launch into a hot mic, and the app's own shortcuts/widgets; AppFunctions is a wrapper to add over the same engine when it opens up.
...
* **The kind of question is parsed too, because a rider asks more than one.** `intent: 'eta' | 'horario'` — a schedule word (`abre`/`cierra`/`opera`/`horario`/`domingos`/`hasta qué hora`/…) makes it a timetable question, which is answerable from the index **alone**: no shard, no position, no live call, so it lands in milliseconds with the phone in airplane mode. "¿A qué hora pasa?" deliberately stays an ETA — it is asking about a bus, not a timetable.
* **A named stop and a named direction are carried as hints.** `stopHint` (after `en`/`por`/`desde`/`estación`/`parada`/`portal`) and `dirHint` (after `hacia`/`sentido`/`rumbo`) are read from the raw tokens, since the markers are themselves stopwords for name matching. Both are *hints*: the ETA engine uses one only when it matches real data on the matched route, so a mis-parse ("por el F19" → stopHint "f19") degrades to the plain nearest-stop answer rather than to a wrong one.
