...
* **The watchdog is for a recognizer that went quiet, not for a rider mid-sentence.** It is re-armed from `onBeginningOfSpeech` (`SPEAKING_TIMEOUT_MS` 15 s). A 12 s ceiling that started while the splash was still up used to expire *over someone audibly speaking* and hand back "no escuché nada".
