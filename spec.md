...
* **Directional wagon merge** (`mergeDirectionalWagons`): A TransMilenio route serves a station in **both directions** (if B72 stops here so does H72; if number 7 stops here it stops both ways). The source data files each direction on its own platform/wagon, so the two directions of one route end up split — wagon A has `B50`/`7`, wagon B has `C50`/`4`. After resolution, wagons that share a **lettered route number** (B50 ↔ C50 → number 50) are the two directions of the same corridor and are **merged into one section**, listing each route once (pairs together: `B50`+`C50`, plus number-only `4`/`6`/`7`). Wagons that share no lettered number are different corridors (e.g. Nariño A=`{3,B72,H72}` vs B=`{B75,C15,H15,H75}`) and stay separate. A troncal code already shown in an earlier section is never repeated, so **no route ever appears in two sections** (no duplication). Verified on the catalog: Escuela Militar A/B → one section `{4,6,7,B11,B50,C50,D24,G11,J24}`, Nariño unchanged, 62 stations merged, **0 troncal codes in more than one section network-wide**.
