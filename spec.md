...
* **Directional route completion** (`completeDirectionalSiblings`): A TransMilenio route serves a station in **both directions** (if B72 stops here so does H72; if number 7 stops here it stops both ways). The source data files each direction on its own platform/wagon, so a wagon lists only one side (`C50` without `B50`; number-only `7` on one platform, `4` on the other). After resolution, wagons that share a **lettered route number** (e.g. B50/C50 → number 50) are recognized as the two directions of one corridor, and their full troncal route set is **unioned** — completing both lettered pairs (B50/C50, B11/G11, D24/J24) and number-only fácil routes (4/7). Wagons that share no lettered number are different corridors (e.g. Nariño A=`{3,B72,H72}` vs B=`{B75,C15,H15,H75}`) and are left untouched. A code already present is never re-added (display also de-dupes by código), so no route is duplicated. Verified on the catalog: Escuela Militar A/B → symmetric, Nariño unchanged, 861 codes completed across 63 stations, 0 new duplicates.
