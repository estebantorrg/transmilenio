...
* The rider's walk is folded in, so the verdict is actionable rather than a bare number: `sal-ya` / `alcanzas` / `no-alcanzas` / `sin-buses` per direction, and `ok` / `lejos` / `sin-geometria` / `fuera-de-servicio` / `cerrado-hoy` for the route. Nothing beyond `ACCESS_MAX_M` (1500 m, the router's access radius) is offered as an ETA — the honest answer there is the distance.
...
**Utterance → código** (`client/src/services/voiceMatch.ts`, tables in `voiceSpanish.ts`). A recognizer never returns "F19"; it returns "efe diecinueve", "la efe 19", "ef 19", or the destination. No NLU — the search space is 783 códigos, so a letter/number table plus edit distance beats a model on accuracy *and* on the budget (measured **~5 ms** per utterance over the whole index, §1 sub-100 ms).
...
* Scores: 1.0 exact código, 0.85 base código (§5.4.3 Ciclovía suffix), 0.6 one edit away, 0.5–0.75 by name/origin/destination token overlap. A lone shared word only counts at ≥6 characters ("banderas" yes, "casa" no). Nothing below **`MIN_VOICE_CONFIDENCE` = 0.5** is returned: the flow says "no te entendí" and the rider repeats one word, which costs a second — answering the wrong route costs a bus.
