...
* **UI/UX** — website (`ui/planner.ts`, `index.html`) and app (`client/mobile/src/views/planner.ts`):
  ...
  * A **Horarios** control: `Todas las rutas` (default — every connection, each labelled with its own schedule) / `Solo en servicio` (the hard filter). Website: a switch beside Salida, mirrored into the deep link as `s=1` (the default stays out of the URL). App: a chip pair in the same options block.
