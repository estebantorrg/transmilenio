...
* `POST /api/arrivals`: real-time arrivals/ETAs at a paradero (`{paradero:"<cenefa>"}` → `{arrivals:[...]}`), reusing the live-bus transport cascade. Never hard-fails — empty list on outage (§5.8 Host 2).
...
**Shipped from this inventory:** real-time arrivals (`POST /api/arrivals`, §5.5.1) and the recharge-point POI layer (`GET /api/recarga-points`, §5.5.1) are implemented on both clients. **Ruled out:** the official OTP journey planner — see Host 3 above. **Still open:** station↔route lookups straight from `loader.php` (`getParaderosList`, `searchRutasByEstacionTroncales`) instead of re-deriving from ArcGIS. **Correction:** card balance is no longer server-only — read it offline via NFC (§5.5.1b).
