...
export const ICONOS = {
  ...
  escalera: {
    ...
    bg: '#0E0E10',
    // Hand-drawn, like the ramp it is the twin of. The sheet draws the two
    // as one pair — the same arrow up one way and down the other, over a
    // slope for the ramp and over steps for the stairs — and Material's
    // `stairs`, a staircase in a box, breaks that pairing on a sheet that
    // prints both.
    svg:
  ...
  banos: {
    label: 'Baños',
    bg: '#0E0E10',
    // Our own, drawn from primitives like the lift and the evacuation sign: the
    // two figures either side of a divider, which is what the sheets print
    // where there are toilets and what their legend never names. For a long
    // time it was simply left off the drawing for want of a glyph — at
    // Bicentenario and AV. 1° de Mayo, the two sheets that print one. Built
    // for eighteen pixels: solid bodies, the woman told apart by her dress,
    // nothing thinner than the stairs' own stroke.
    vb: '0 0 24 24',
    svg:
      '<circle cx="6.3" cy="3.9" r="2" fill="' + W + '"/>' +
      '<path d="M4.2 6.9h4.2a1.4 1.4 0 0 1 1.4 1.4v5.4h-1.5v7.2H6.9v-5.3h-1.2v5.3H4.3v-7.2H2.8V8.3a1.4 1.4 0 0 1 1.4-1.4z" fill="' + W + '"/>' +
      '<rect x="11.35" y="2.2" width="1.3" height="19.6" rx="0.4" fill="' + W + '"/>' +
      '<circle cx="17.7" cy="3.9" r="2" fill="' + W + '"/>' +
      '<path d="M16.3 6.9h2.8c.6 0 1.1.4 1.3 1l2 6.8h-2.6v6.2h-1.4v-6.2h-.4v6.2h-1.4v-6.2h-2.6l2-6.8c.2-.6.7-1 1.3-1z" fill="' + W + '"/>',
  },
  tbici: {
    label: 'TransMiBici',
    bg: '#0E0E10',
